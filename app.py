...
# ---------- Local Auth ----------
def local_register(username, password, role="Farmer"):
...
def local_login(username, password):
    ...
    return user and user["password"] == hash_password(password), user["role"] if user else None
...
if not st.session_state.logged_in:
    ...
    if st.button("Submit"):
        if kind=="Register":
            ...
            ok, role = local_login(username_input,password_input)
            if ok:
                st.session_state.logged_in=True
                st.session_state.username=username_input
                st.session_state.role=role
                st.success(f"Logged in locally as {username_input} ({role})")
            else:
...
if st.session_state.logged_in and st.session_state.role=="Farmer":
    ...
    if uploaded:
        ...
        if st.button("🔍 Detect"):
            ...
            if supabase and table_exists(TABLE_DETECTION):
                try:
...
if st.session_state.logged_in and st.session_state.role=="Admin":
    ...
    if supabase and table_exists(TABLE_DETECTION):
        try:
            records_res = supabase.table(TABLE_DETECTION).select("*").order("timestamp", desc=True).execute()
